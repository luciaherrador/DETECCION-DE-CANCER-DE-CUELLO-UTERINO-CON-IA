...
class GroupedCEExplainer(LocalBBExplainer):
    ...
    def _partition_sample(self, x: np.ndarray, n: int, d: int = 0):
        """
        First recursive binary partition using feature values sampling
        of the high dimensional data.
        """
        # Check if current partitioning depth exceeds allowed based on target sample size
        if 2**d > n or x.shape[0] <= 1:
            # If we exceed the partitioning depth or x has one or fewer samples, sample directly
            n_samples = min(n, x.shape[0])
            i = np.random.choice(x.shape[0], n_samples, replace=True)
            return x[i]
        ...
        # Partition the data based on the median of the selected dimension
        x_lo = x[x[:, dim] <= y]
        x_hi = x[x[:, dim] > y]
        ...
        if x_lo.shape[0] > 0:
            ...
            if s1 is not None and len(s1.shape) == 1:
                s1 = s1[np.newaxis, ...]

        if x_hi.shape[0] > 0:
            s2 = self._partition_sample(x_hi, n, d=d + 1)
            if s2 is not None and len(s2.shape) == 1:
                s2 = s2[np.newaxis, ...]

        # Combine the results ensuring no None values are used
        if s1 is not None and s2 is not None:
            ...
            s = x  # Fallback in case both are None, use the original data
